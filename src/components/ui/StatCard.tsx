...
export function StatCard({ label, value, subtext, accent = "var(--accent-primary)" }: StatCardProps) {
  return (
    <div
      className="hud-clipped flex flex-col border border-[var(--border-muted)] bg-[var(--bg-panel)] p-5 transition-colors hover:border-[var(--accent-primary)]/40"
    ...
    >
      ...
      {subtext && (
        <p className="mt-2 border-t border-[var(--border-muted)]/50 pt-2 font-mono text-xs text-[var(--text-muted)]">
          {subtext}
        </p>
      )}
    </div>
