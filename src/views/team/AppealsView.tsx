...
export function AppealsView() {
  ...
  const { appeals, mySubmissions } = data;
  ...
  return (
    <div className="p-[var(--space-xl)] max-w-[var(--container-max)] mx-auto hud-lattice min-h-[calc(100vh-4rem)]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-[var(--border-muted)] pb-6">
        ...
        <Button variant="ghost" onClick={() => actions.refetch()} className="text-xs font-mono shrink-0">
                ...
                <form onSubmit={actions.handleCreateAppeal} className="flex flex-col gap-4">
                  <div className="flex flex-col gap-1.5 w-full">
                    ...
                    <select
                      ...
                      onChange={(e) => actions.setSubmitResultId(e.target.value)}
                      required
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell align="center">
                        <Button
                          variant="ghost"
                          onClick={() => {
                            actions.setDetailModal(item);
                            actions.setResponseText(item.response ?? "");
                          }}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center px-4">
          <Card className="w-full max-w-2xl p-6 bg-[var(--bg-panel)] hud-clipped border-[var(--color-warning)] space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-[var(--border-muted)] pb-4">
              ...
              <button onClick={() => actions.setDetailModal(null)} className="text-[var(--text-muted)] hover:text-white">
              ...
              {isEC && (
                <div className="space-y-2 pt-2 border-t border-[var(--border-muted)]">
                  ...
                  <textarea
                    rows={3}
                    value={responseText}
                    onChange={(e) => actions.setResponseText(e.target.value)}
            ...
            <div className="flex items-center justify-between gap-3 pt-4 border-t border-[var(--border-muted)] font-mono text-xs">
              <Button variant="ghost" onClick={() => actions.setDetailModal(null)}>
                Đóng
              </Button>
              ...
              {isEC && detailModal.status === AppealStatus.Pending && (
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() => actions.handleRespond(AppealStatus.Rejected)}
                  ...
                  <Button
                    disabled={!responseText.trim() || isResponding}
                    onClick={() => actions.handleRespond(AppealStatus.Approved)}
