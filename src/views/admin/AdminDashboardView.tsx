...
import { Plus, Users, School, Activity, Shield, UserCheck, X, CheckCircle2, Sparkles } from "lucide-react";
import Link from "next/link";
import { ApiMissingDataBadge } from "@/components/ui";
import { useAdminDashboardViewModel } from "@/viewModels/admin/useAdminDashboardViewModel";
...
export const AdminDashboardView: React.FC = () => {
  const { state, data, actions } = useAdminDashboardViewModel();
      ...
      <main className="flex-1 max-w-[var(--container-max)] w-full mx-auto px-6 py-8 space-y-6">
        ...
        <Card className="p-6 space-y-4 bg-[var(--bg-panel)] hud-clipped border-[var(--border-muted)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Button
              variant="ghost"
              onClick={actions.handleSetupDemoEvents}
              disabled={isSettingUpDemoEvents}
              className="font-mono text-xs flex flex-col items-start gap-1 p-4 h-auto text-left"
            >
              <span className="flex items-center gap-1.5 font-bold"><Sparkles className="w-3.5 h-3.5" /> Nộp → Chấm → Công bố</span>
              <span className="text-[var(--text-muted)] font-normal">Khác Full 100%. 1 sự kiện: Team 1 trống, Team 2 đã chấm sẵn</span>
              {isSettingUpDemoEvents && <span className="text-[var(--accent-primary)]">Đang tạo...</span>}
            </Button>
            <Button
              variant="ghost"
              onClick={actions.handleSetupDemoAppealEvent}
            ...
            <Button
              variant="primary"
              onClick={actions.handleSetupFullEventDemo}
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td className="text-center">
                          <Button
                            variant="ghost"
                            onClick={() => actions.handleOpenAssignModal(ev)}
          ...
          <div className="fixed inset-0 bg-black/75 flex items-center justify-center p-4 z-50 animate-fade-in">
            <Card className="w-full max-w-lg p-6 bg-[var(--bg-panel)] border border-[var(--accent-coordinator)] space-y-4 relative hud-clipped">
              ...
              {assignSuccessMessage ? (
                ...
                <form onSubmit={actions.handleAssignEc} className="space-y-4 pt-2">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      value={ecEmail}
                      onChange={(e) => actions.setEcEmail(e.target.value)}
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="ghost" type="button" onClick={() => actions.setSelectedEvent(null)}>
                      Hủy Bỏ
                    </Button>
