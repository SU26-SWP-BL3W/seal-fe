...
export function RegisterView() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setError('Lỗi khi đăng ký với Google');
      setIsLoading(false);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-[#0a0f1d] text-[#f1f5f9] antialiased flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md mx-4 p-1">
        ...
        {/* Success Message */}
        {successMessage && (
          <div className="mb-6 p-4 bg-green-500/10 border border-green-500/50 rounded-sm text-green-400 text-sm font-mono">
            ✓ {successMessage}
          </div>
        )}
