...
import { computeEventStatus, STATUS_LABEL, STATUS_TONE, STATUS_DOT_VAR, type EventItem } from "@/viewModels/public/eventsMetadata";
import { Shield, Settings, Activity, Users, CalendarPlus, Trash2, Edit3, Award, FileText, CheckCircle2, Sliders, ExternalLink, EyeOff, Rocket } from "lucide-react";
import Link from "next/link";
import { useCoordinatorDashboardViewModel } from "@/viewModels/coordinator/useCoordinatorDashboardViewModel";
import type { MyEventModel } from "@/repositories/eventsRepository";
...
export const CoordinatorDashboardView: React.FC = () => {
  const { state, data, actions } = useCoordinatorDashboardViewModel();
                    ...
                    <div className="border-t border-[var(--border-muted)] pt-4 space-y-2.5">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button
                          ...
                          onClick={() => actions.handleTogglePublish(id, isPublished, name)}
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <button
                          type="button"
                          onClick={() => {
                            actions.setDeleteTargetId(id);
                            actions.setDeleteTargetName(name);
                          }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[var(--bg-panel)] border border-[var(--color-danger)] p-6 max-w-md w-full hud-clipped space-y-4 shadow-[0_0_30px_rgba(239,68,68,0.3)]">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <Button
                variant="ghost"
                onClick={() => actions.setDeleteTargetId(null)}
                disabled={isDeleting}
                className="text-xs font-mono"
              >
                HỦY BỎ
              ...
              <Button
                variant="primary"
                onClick={actions.handleDeleteEvent}
                disabled={isDeleting}
