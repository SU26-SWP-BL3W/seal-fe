...
export function CoordinatorProfilesView() {
  ...
  // Edit user form state inside detail modal
  ...
  const [localUsersOverride, setLocalUsersOverride] = useState<Map<string, Partial<User>>>(new Map());
  ...
  }, [activeTab, selectedEventId, selectedTrackId, selectedAccountType, searchQuery, sortBy]);
  ...
  const allUsers = useMemo(() => {
    ...
    return rawList.map((u) => {
      ...
      if (localUsersOverride.has(id)) {
        return { ...u, ...localUsersOverride.get(id) };
      }
      return u;
    });
  ...
  const filteredUsers = useMemo(() => {
    return allUsers.filter((u) => {
      ...
      if (activeTab === "pending") {
        ...
        if (u.isApproved || (!u.rejectionCount && !u.isRejected)) return false;
      }

      // 2. Track Filter
      if (selectedTrackId !== "all") {
        if (u.trackId !== selectedTrackId) return false;
      }

      // 3. Event Filter
      if (selectedEventId !== "all") {
        if (u.eventId && u.eventId !== selectedEventId) return false;
      }

      // 4. Account Type Filter
      if (selectedAccountType === "student_fpt") {
        if (!u.isFpt || !u.isStudent) return false;
      } else if (selectedAccountType === "student_other") {
        if (u.isFpt || !u.isStudent) return false;
      ...
      if (searchQuery.trim()) {
        ...
        const name = (u.fullName || (u as any).FullName || "").toLowerCase();
        const email = (u.email || (u as any).Email || "").toLowerCase();
        const code = (u.studentCode || (u as any).StudentCode || "").toLowerCase();
        const school = (u.schoolName || (u as any).SchoolName || "").toLowerCase();
        const team = (u.teamName || "").toLowerCase();
        const track = (u.trackName || "").toLowerCase();
        if (!name.includes(q) && !email.includes(q) && !code.includes(q) && !school.includes(q) && !team.includes(q) && !track.includes(q)) {
          return false;
        }
      }
  ...
  }, [allUsers, activeTab, selectedTrackId, selectedEventId, selectedAccountType, searchQuery]);
      ...
      <div className="max-w-7xl mx-auto">
        {isUsersError ? (
          ...
          <div className="border border-[var(--border-muted)] bg-[var(--bg-panel)] hud-clipped overflow-hidden shadow-lg">
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => handleToggleSelectOne(userId)}
                            className="cursor-pointer accent-cyan-400 w-3.5 h-3.5"
                          />
                        </td>
                        ...
                        <td className="p-3">
                          <div
                            onClick={() => setPreviewModal(user)}
                            className="relative w-14 h-10 bg-black/40 border border-[var(--border-muted)] hover:border-cyan-400 rounded overflow-hidden flex-shrink-0 cursor-pointer group transition-all"
                            title="Click để phóng to ảnh thẻ sinh viên"
                          >
            ...
            ───────────────────────────────────────────────────────────── */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-[var(--border-muted)] bg-[var(--bg-input)] font-mono text-xs">
              <div className="flex items-center gap-3">
                <span className="text-[var(--text-muted)]">
                  Hiển thị: <strong>{(currentPage - 1) * pageSize + 1}</strong> – <strong>{Math.min(currentPage * pageSize, totalItems)}</strong> trong tổng số <strong>{totalItems}</strong> người dùng
                </span>
                ...
                <select
                  ...
                  className="px-2 py-1 bg-[var(--bg-panel)] border border-[var(--border-muted)] text-[var(--text-primary)] rounded text-xs"
                >
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="p-1.5 bg-[var(--bg-panel)] border border-[var(--border-muted)] hover:border-slate-500 text-slate-300 rounded disabled:opacity-40 cursor-pointer"
                  title="Trang đầu"
                >
                ...
                <span className="px-3 py-1 bg-black/40 border border-slate-700 text-cyan-300 font-bold rounded">
